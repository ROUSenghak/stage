...
The Cox proportional hazards model achieves an in-sample concordance of 0.6524 (C-index);
the best-fitting parametric model is log-normal with AIC = 7\,119.8 and
concordance 0.6723. Statistically significant predictors are
\texttt{declared\_duration\_months} (HR = 0.991, $p < 0.001$) and
\texttt{start\_year} (HR = 1.094, $p < 0.001$). A sensitivity analysis
restricting to high-confidence links (composite $\geq 0.50$, 494 events)
yields a KM median of 52.7 months and Cox concordance of 0.6436, supporting
...
\begin{enumerate}
  \item \textbf{Raw notice corpus.} The API extraction and flattening step
        produces \texttt{boamp\_full\_flat.csv} with 3,181 BOAMP notices and 30
  ...
  \item \textbf{Survival handoff.} The final file,
        \texttt{boamp\_phase2\_survival.csv}, has one row per eligible contract,
...
\small
\begin{tabularx}{\textwidth}{@{}P{3.4cm}P{3.1cm}X X@{}}
\toprule
...
The final dataset contains 697 linked renewals and 403 unlinked eligible
...
\begin{itemize}
  \item \texttt{event = 1}: the algorithm found a plausible later renewal
        notice; \texttt{renewal\_duration\_months} is observed.
  \item \texttt{event = 0}: no accepted renewal was observed before study end;
        \texttt{censoring\_duration\_months} is observed.
  \item \texttt{observed\_duration\_months}: always filled, using the renewal
...
\texttt{cpv\_used\_in\_score} column records, per pair, whether CPV contributed.
...
\texttt{composite\_score}   & float   & $S(i,j^*)$. Mean: 0.579, range: 0.245--0.998. \\
\texttt{text\_similarity}   & float   & $s_{\text{text}}(i,j^*)$. Mean: 0.517, range: 0.204--1.000. \\
\texttt{cpv\_match\_score}  & float   & $s_{\text{cpv}}(i,j^*)$. Mean: 0.301, range: 0.000--1.000 (over the 646 links with a CPV; NaN for the 51 missing-CPV links). \\
...
\texttt{temporal\_score}    & float   & $s_{\text{temp}}(i,j^*)$. Mean: 0.706, range: 0.006--1.000. \\[6pt]
...
\texttt{second\_best\_composite\_score} & float & $S(i,j^{(2)})$: composite score of the runner-up. NaN for single-candidate sources. \\
\texttt{score\_margin}        & float & $\Delta = S(i,j^{(1)}) - S(i,j^{(2)})$. Best-minus-second-best gap; NaN when only one candidate. \\
\texttt{n\_candidates\_for\_source} & int & Number of filtered candidates for the source (135 sources have exactly one; 562 have $\geq 2$). \\
\texttt{single\_candidate\_match} & bool & True when the source had a single candidate (135 rows); margin undefined, not auto-promoted to strict. \\
\texttt{high\_confidence\_strict} & bool & True iff \texttt{event}=1 $\wedge\ S \geq 0.70 \wedge \Delta \geq 0.05$ (90 rows). Conservative sensitivity flag for Model C (Section~\ref{sec:margin}). \\
\end{longtable}
\normalsize
...
\begin{table}[H]
\centering
\caption{Descriptive statistics of matching scores (event=1 rows only, n=697; \texttt{cpv\_match\_score} over the 646 links with a non-missing CPV)}
...
(composite $< 0.50$, red, $n = 203$), MEDIUM ($0.50$--$0.70$, orange, $n = 372$),
HIGH ($\geq 0.70$, green, $n = 122$). The dashed line marks the mean (0.579).
...
\texttt{TEXT\_MISMATCH}        &  32 &  7.9\% & Temporal candidates existed but all had $s_{\text{text}} < 0.20$. Text was too dissimilar even at the semantic level. \\[4pt]
\texttt{CPV\_MISMATCH}         &  32 &  7.9\% & Temporal candidates existed, but all candidates from the same buyer had a \emph{valid but incompatible} CPV division ($s_{\text{cpv}} = 0$); sources with a missing CPV are excluded from this label. A diagnostic warning, not a CPV hard gate in the final scoring rule. \\
\midrule
...
\emph{composite} score $S(i,j^*) \geq 0.50$ (494 events). This analysis informs the
...
= HIGH ($S \geq 0.70$, $n = 122$); orange = MEDIUM ($0.50 \leq S < 0.70$,
$n = 372$); red = LOW ($S < 0.50$, $n = 203$); light blue = censored
(event = 0, $n = 403$). The HIGH and MEDIUM tiers together cover 494 of the
697 linked contracts (70.9\%), providing a reliable core dataset for survival
...
HIGH: $S \geq 0.80$ and margin $\geq 0.05$ ($n = 53$, 7.6\% of 697 events);
MEDIUM: $0.70 \leq S < 0.80$ or ($S \geq 0.70$ and margin $< 0.05$)
($n = 170$, 24.4\%); LOW: $S < 0.70$ ($n = 339$, 48.6\%); SINGLE:
single-candidate links with undefined margin ($n = 135$, 19.4\%).
The strict high-confidence threshold used in Model~C (Section~\ref{sec:reliability})
corresponds to the HIGH and SINGLE tiers being distinguished from the rest.}
...
\begin{enumerate}
  \item \textbf{Model A --- Primary}: all 697 events (composite $\geq 0.20$, text $\geq 0.20$).
  \item \textbf{Model B --- Sensitivity check}: 494 events only (drop LOW tier, i.e.\ composite $\geq 0.50$).
  \item \textbf{Model C --- Strict high-confidence}: 90 events only, keeping
...
marks the 0.05 threshold below which 48.6\% of pairs are considered
...
\textit{clear winner} zone (28.1\% of pairs). These ambiguous pairs are
...
The 48.6\% ambiguous-margin rate does not mean the algorithm chose wrongly in
...
Of the 697 links, \textbf{90} qualify. Single-candidate sources ($n = 135$)
...
Ambiguous score margin ($<0.05$) & 48.6\% of multi-cand. & Structural; not tunable \\[4pt]
Strict high-confidence links & 90 / 697 (12.9\%) & $S \geq 0.70$ \& $\Delta \geq 0.05$; Model~C core \\[4pt]
...
all 1,100 contracts. The global KM median survival is \textbf{48.1 months},
...
concordance (C-index) is \textbf{0.6524}, indicating moderate in-sample ranking
ability beyond a random ranking.

\begin{table}[H]
\centering
\caption{Multivariate Cox proportional hazards model (C-index = 0.6524)}
\label{tab:cox_multi}
\small
...
\midrule
Event criterion & composite $\geq 0.20$ & composite $\geq 0.50$ & $S \geq 0.70$ \& $\Delta \geq 0.05$ \\
Events & 697 & 494 & 90 \\
...
48.1 to 52.7 months, consistent with the excluded LOW-tier links being
...
all but the 90 strict links, the event rate collapses to 8.2\% and the KM
median is no longer reached, so the apparent \emph{level} of renewal risk drops
sharply, exactly as expected when most proxy events are removed. The decisive
observation is that the Cox concordance remains stable across all three
definitions (0.652, 0.644, 0.630): the smaller event counts reduce power but the
\emph{ranking} of renewal risk by covariate is robust. The qualitative
...
Model~C is a conservative lower bound, not a corrected ground-truth estimate.
...
\caption{Kaplan--Meier comparison: Model A (all 697 linked contracts, median
48.1 mo) vs.\ Model B (494 high/medium-confidence contracts, median 52.7 mo)
vs.\ Model C (90 strict high-confidence links, median not reached). Models~A
...
concordance across all three (0.652, 0.644, 0.630) confirms that lower-confidence
...
score-acceptance thresholds from 0.35 (baseline) to 0.90, plus the
strict high-confidence variant.
...
\scriptsize
\begin{tabular}{@{}p{4.2cm}rrrrrrr@{}}
\toprule
\textbf{Scenario} & \textbf{events} & \textbf{rate\%} & \textbf{KM med.} &
\textbf{Cox C} & \textbf{HR dur.} & \textbf{HR yr} \\
\midrule
Baseline (all, $\geq 0.35$)               & 697 & 63.4 & 48.1 & 0.6524 & 0.992 & 1.094 \\
Score $\geq 0.50$                          & 494 & 44.9 & 52.7 & 0.6436 & 0.992 & 1.074 \\
Score $\geq 0.60$                          & 251 & 22.8 & $\infty$ & 0.6221 & 0.995 & 1.068 \\
Score $\geq 0.70$                          & 122 & 11.1 & $\infty$ & 0.6317 & 0.995 & 1.035 \\
Score $\geq 0.80$                          &  66 &  6.0 & $\infty$ & 0.5935 & 0.999 & 1.015 \\
Score $\geq 0.90$                          &  22 &  2.0 & $\infty$ & 0.5558 & 1.001 & 1.013 \\
Strict H.C.\ ($S\!\geq\!0.70$, $\Delta\!\geq\!0.05$) & 104 & 9.5 & $\infty$ & 0.6375 & 0.995 & 1.046 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key observations.} (i) The Cox C-index stays above 0.59 across all
thresholds, including the extreme $\geq 0.90$ case (22 events). (ii) The
...
falls below $\approx$23\%, because the study window is too short to observe the
median survival under such sparse events. (iv) The strict H.C.\ variant (104
events including single-candidate links with $S \geq 0.70$) gives
C~= 0.638 --- higher than the strict\_hc\_only variant (90 events,
C~= 0.630) that excludes single-candidate links.
...
\begin{subfigure}{0.49\textwidth}
  ...
  C-index plateau between 0.35 and 0.70 (range 0.622--0.652) confirms that
  adding lower-confidence links does not degrade ranking ability.}
\end{subfigure}
...
$N$ from 1{,}100 to 763). \texttt{strict\_hc\_only} keeps all 1{,}100
contracts but censors all but the 90 strict high-confidence events.}
\label{tab:exclusion}
\small
...
Strict H.C.\ only      & 1{,}100 &  90 &  8.2 & $\infty$ & 0.6295 \\
\bottomrule
\end{tabular}
\end{table}

Excluding uncertain links shifts the KM median upward to 59.0 months and the
...
persists. The drop in C-index relative to the baseline (0.652 $\to$ 0.601)
...
across the 697 accepted links. The temporal score is the highest-mean component;
the CPV match score is the lowest, reflecting frequent reclassification across
procurement cycles. The score margin median (0.052) is just above the 0.05
ambiguity threshold, meaning roughly half of multi-candidate links are near the
boundary.

\begin{table}[H]
\centering
\caption{Score component distributions for the 697 event-1 links. CPV
count (646) is lower because 51 links have a missing CPV on at least one side.
Score margin is defined only for 562 multi-candidate links.}
...
Score margin ($\Delta$) & 562 & 0.088 & 0.105 & 0.019 & 0.052 & 0.108 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Composite score vs.\ declared contract duration (months) for the 697
...
\toprule
\textbf{Variable} & \textbf{event = 1 ($n$ = 697)} & \textbf{event = 0 ($n$ = 403)} &
\textbf{SMD / $\Delta$} \\
\midrule
Declared duration (months) & 32.3 $\pm$ 17.0 & 30.0 $\pm$ 18.6 & 0.128 \\
...
\midrule
N candidates considered    & 7.7 $\pm$ 8.0  & 0.0 $\pm$ 0.0  & 1.362 (structural) \\
\bottomrule
\end{tabular}
\end{table}
...
indicating good balance. The large SMD for N\_candidates (1.36) is structural
...
\begin{enumerate}[leftmargin=*, label=\textbf{F\arabic*.}]
  ...
  \item \textbf{Log-normal AFT model preferred.} Among parametric models, the
        log-normal achieves the best trade-off between fit (AIC = 7\,119.8) and
  ...
  \item \textbf{Sensitivity check supports qualitative stability.} Restricting to
        high/medium-confidence links (Model B) shifts the survival curve upward
        so the median is no longer reached, but leaves the covariate effects
        qualitatively unchanged. The primary model (Model A) is retained as the
        official result.
...
\begin{table}[H]
\centering
\caption{Operational prediction output files (in \texttt{reports/tables/survival/})}
\label{tab:predfiles}
\footnotesize
...
\midrule
\texttt{renewal\_risk\_\allowbreak 12\_24\_\allowbreak months.csv} & 1{,}097 & Contract-level predictions: one row per scored contract. Main columns: \texttt{contract\_id}, \texttt{buyer\_key}, \texttt{category\_label}, \texttt{age\_months}, \texttt{p\_renewal\_12m}, \texttt{p\_renewal\_24m}, \texttt{risk\_tier}, \texttt{model\_used}. \\
\texttt{buyer\_renewal\_\allowbreak risk\_ranking.csv} & 306 & Buyer-level ranking by expected renewals. Main columns: \texttt{buyer\_key}, \texttt{n\_contracts}, \texttt{expected\_renewals\_12m}/\texttt{\_24m}, \texttt{max}/\texttt{mean\_p\_renewal\_12m}, \texttt{high\_risk\_contracts\_count}. \\
\texttt{segment\_renewal\_\allowbreak risk\_ranking.csv} & 11 & Segment-level ranking by expected renewals. Main columns: \texttt{category\_label}, \texttt{n\_contracts}, \texttt{expected\_renewals\_12m}/\texttt{\_24m}, \texttt{mean\_p\_renewal\_12m}/\texttt{\_24m}, \texttt{high\_risk\_contracts\_count}. \\
\bottomrule
\end{tabular}
\normalsize
\end{table}
