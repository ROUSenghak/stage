...
\begin{enumerate}
  ...
  \item \textbf{Model B --- Sensitivity check}: 553 events only (drop LOW tier, i.e.\ composite $\geq 0.50$).
  \item \textbf{Model C --- Strict high-confidence}: 132 events only, keeping
...
Of the 697 links, \textbf{132} qualify. Single-candidate sources ($n = 135$)
...
Strict high-confidence links & 132 / 697 (18.9\%) & $S \geq 0.70$ \& $\Delta \geq 0.05$; Model~C core \\[4pt]
...
Events & 697 & 553 & 132 \\
Censored & 403 & 547 & 968 \\
Event rate & 63.4\% & 50.3\% & 12.0\% \\
KM median survival & 48.0 mo & 53.0 mo & not reached \\
Cox C-index & 0.6528 & 0.6202 & 0.6155 \\
\bottomrule
\end{tabular}
\end{table}
...
more aggressive --- by censoring all but the 132 strict links, the event rate
collapses to 12.0\% and the KM median is no longer reached, so the apparent
...
essentially unchanged across all three definitions (0.653, 0.620, 0.616): the
...
Model B (553 high/medium-confidence contracts) vs.\ Model C (132 strict
...
lower-confidence links add noise but do not introduce systematic bias.}
