...
\lhead{\small Gigalis Internship --- Phase 1}
\cfoot{\thepage}

\title{
  \textbf{BOAMP Procurement Renewal Linking}\\[0.4em]
  \large Final Technical Report --- Phase 1\\[0.3em]
  \normalsize Data Audit, Preprocessing, Mathematical Formulation, and Renewal Event Construction
}
...
\begin{abstract}
This report presents the final Phase 1 methodology for reconstructing renewal
links in BOAMP public procurement data. BOAMP notices do not contain an explicit
field identifying whether a call-for-tender (\textit{appel d'offre}) renews a
previous contract. The renewal relationship must therefore be inferred from
observable signals: buyer identity, contract description, CPV classification,
and timing relative to the expected contract end date. Starting from 1,933
\textit{APPEL\_OFFRE} notices over 2015--2024, the pipeline defines 1,100
eligible source contracts and links 697 of them to a later renewal candidate,
giving a final linking rate of 63.4\%. The previous baseline approach linked
219 notices out of all 1,933 \textit{APPEL\_OFFRE} notices (11.3\%); the final
method uses the narrower denominator of 1,100 notices whose expected renewal
date is observable within the study period. The notebook linking step first
produces \texttt{boamp\_renewal\_links.csv}, then the official BOAMP-only
Phase-2 handoff is exported as \texttt{boamp\_phase2\_survival.csv}. This
final file contains one row per eligible source contract and an
\texttt{event} variable for survival modeling.
\end{abstract}
...
largest remaining weakness is buyer identification: only 5.9\% of buyers are
identified by SIRET, so most grouping relies on normalised buyer names. This can
fragment the same public entity into several buyer keys and create false
negatives.
...
\scriptsize
\begin{longtable}{@{}P{3.8cm}P{1.3cm}P{1.3cm}P{8.0cm}@{}}
...
\texttt{buyer\_ville}    & string  & 100\%  & City name of the buyer. \\
...
\texttt{type\_marche}    & string  & 91.6\% & Contract type: SERVICES (53.6\%), FOURNITURES (26.2\%), TRAVAUX (12.0\%), mixed. \\
\texttt{type\_avis}      & string  & 100\%  & Notice subtype codes (e.g.\ \texttt{5|1|}). \\
...
\texttt{cpv\_principal}  & float   & 96.8\% & Raw CPV code (8-digit integer) as published. \\
\texttt{cpv\_all}        & string  & 100\%  & Pipe-delimited list of all CPV codes for the notice. \\
\texttt{cpv\_clean}$^\dagger$ & float & 96.8\% & Cleaned primary CPV code. Generic codes (ending in \texttt{000000}) flagged separately. \\
\texttt{cpv\_div2}$^\dagger$  & float & 96.8\% & First 2 digits of CPV (division-level, e.g.\ 72 = IT services). Range: 15--98. \\
\texttt{cpv\_class4}$^\dagger$ & float & 96.8\% & First 4 digits of CPV (class-level granularity). \\
\texttt{category\_id}$^\dagger$ & string & 100\% & Internal category code (CAT01--CAT11) derived from CPV. \\
...
\multicolumn{4}{@{}P{14.4cm}@{}}{\scriptsize $^\dagger$ Column added or derived by the cleaning pipeline. Not present in the original BOAMP API response.} \\
\multicolumn{4}{@{}P{14.4cm}@{}}{\scriptsize $^*$ \texttt{date\_attribution} is 0\% for AO notices; it is populated on ATTRIBUTION notices and used indirectly via \texttt{annonce\_lie}.} \\
\end{longtable}
\normalsize
...
The 11 internal categories are derived from the CPV 2-digit division and cover
exclusively the ICT/digital services scope of the Gigalis portfolio.
...
The dominance of name-based buyer keys (94.1\%) is the primary structural
...
\small
\begin{tabular}{@{}p{3.6cm}crcr p{2.6cm}@{}}
\toprule
...
four integers --- all multiples of 12 months --- and even the 5th and 6th most
common values (60 and 72 months) represent administrative 5-year and 6-year
ceilings, not empirically measured durations.
...
1 & SIRET coverage low
  & Only 5.8\% of AO have a SIRET; the rest use normalised name as buyer key
...
The goal is to construct, for each eligible source notice $i$, a binary outcome
$Y_i$ indicating whether a plausible renewal candidate can be found among later
...
\begin{equation}
  s_{\text{text}}(i,j) = \mathbf{v}_i \cdot \mathbf{v}_j \in [0,1]
\end{equation}
...
\small
\begin{longtable}{p{4.8cm} p{1.5cm} p{7.0cm}}
...
\texttt{buyer\_key}         & string  & Buyer identifier (\texttt{SIRET:...} or \texttt{NAME:...}). 308 unique buyers. \\
...
(composite $< 0.50$, red, $n = 169$), MEDIUM ($0.50$--$0.70$, orange, $n = 347$),
HIGH ($\geq 0.70$, green, $n = 181$). The dashed line marks the mean (0.624).
...
\small
\begin{tabular}{p{4.3cm}rrp{7.1cm}}
\toprule
...
raising it to 0.50 yields 309 events (28.1\%). This analysis informs the
...
\begin{enumerate}[leftmargin=*, label=\textbf{L\arabic*.}]
  ...
  \item \textbf{Name-based buyer fragmentation.} 94.2\% of buyer keys are
...
\midrule
HIGH   & $S(i,j^*) \geq 0.70$ & 181 & 26.0\% \\
MEDIUM & $0.50 \leq S(i,j^*) < 0.70$ & 347 & 49.8\% \\
LOW    & $S(i,j^*) < 0.50$ & 169 & 24.2\% \\
\midrule
...
= HIGH ($S \geq 0.70$, $n = 181$); orange = MEDIUM ($0.50 \leq S < 0.70$,
$n = 347$); red = LOW ($S < 0.50$, $n = 169$); light blue = censored
(event = 0, $n = 403$). The HIGH and MEDIUM tiers together cover 528 of the
697 linked contracts (75.8\%), providing a reliable core dataset for survival
...
\begin{enumerate}
  ...
  \item \textbf{Sensitivity check}: 528 events only (drop LOW tier, i.e.\ composite $\geq 0.50$).
...
HIGH + MEDIUM tier share & 528 / 697 (75.8\%) & Reliable core for modeling \\
\bottomrule
\end{tabular}
\normalsize
\end{table}
...
\noindent The notebook produces four output files. The main modeling table is
...
\small
\begin{tabular}{@{}p{6.1cm}>{\raggedleft\arraybackslash}p{1.2cm}p{5.7cm}@{}}
\toprule
File & Rows & Purpose \\
\midrule
...
\begin{enumerate}
  \item \textbf{Phase 2 --- Survival model:} Fit a Cox proportional hazards or
        Kaplan--Meier model on \texttt{boamp\_phase2\_survival.csv} to estimate
        time-to-renewal distributions and identify covariates (CPV, buyer type,
        contract size, procedure type) that predict early or late renewal.
        The survival time is \texttt{observed\_duration\_months}
        ($= \delta(i, j^*)$, the actual gap); the declared duration is a
        covariate, not the target (see Section~\ref{sec:duration_science}).
  \item \textbf{Phase 3 --- Validation:} Manual annotation of a stratified sample
        ...
        the composite score threshold and temporal window.
    \item \textbf{Phase 4 --- Optional enrichment:} Revisit DECP integration only
      ...
      linkage strategy is defined. Any enrichment should remain separate from
      the current Phase-2 handoff until contract-level linkage quality is
      demonstrated.
