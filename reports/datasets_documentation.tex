...
\section*{Mise à jour calibration --- 2026-07-05 (complétée le 2026-07-08)}

Depuis la comparaison de méthodes du 2026-07-08, la version sélectionnée pour
l'analyse de survie BOAMP est le fichier
\texttt{data/processed/boamp\_phase2\_survival\_method\_m2\_balanced.csv}
(1\,210 appels d'offre éligibles, 254 événements de réapparition proxy,
21,0\%; méthode M2 équilibrée, seuil de probabilité 0,65). Le fichier calibré
\texttt{data/processed/boamp\_phase2\_survival\_calibrated\_balanced.csv}
reste la référence conservatrice (M0 équilibrée) : 1\,210 appels d'offre
éligibles et 269 événements (22,2\%, calibration Sentence-Transformer). La
règle équilibrée M0 est : similarité texte $\geq 0{,}50$, score composite
$\geq 0{,}50$, fenêtre temporelle $W=6$, correction des codes CPV génériques,
sans seuil minimum sur la marge.

Les variantes conservées pour sensibilité sont :
\texttt{boamp\_phase2\_survival\_calibrated\_broad.csv} (490 événements,
40,5\%) et \texttt{boamp\_phase2\_survival\_calibrated\_strict.csv}
(79 événements, 6,5\%). Ces événements sont des récurrences proxy identifiées
dans BOAMP, pas des renouvellements légalement certifiés.
...
\begin{itemize}
  \item[---] \texttt{cpv\_clean}, \texttt{cpv\_full8}, \texttt{cpv\_div2}, \texttt{cpv\_group3}, \texttt{cpv\_class4}, \texttt{cpv\_category5}, \texttt{cpv\_is\_missing}, \texttt{cpv\_is\_generic} :
    code CPV normalisé (8 chiffres) et toute la hiérarchie -- division (2), groupe (3), classe (4), catégorie (5 chiffres) -- plus les indicateurs CPV manquant / générique
