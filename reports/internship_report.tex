...
signals, used as the event variable for survival analysis. The current selected
method is \textbf{M0 balanced}, a calibrated composite rule with text similarity
$\geq 0.50$, composite score $\geq 0.50$, $W=6$ months, corrected generic CPV
handling, and no margin floor. On 1{,}210 eligible source contracts it records
\textbf{269 proxy recurrence events} (\textbf{22.2\%}). The older 665-event
output is retained only as the pre-calibration baseline. A new method-comparison
experiment tested M0 against probabilistic linkage (M1) and active-learning-
assisted linkage (M2); M2 balanced is the best alternative candidate but does
not replace M0 in the current report. Under M0 balanced, the Kaplan--Meier
median is not reached, Cox C-index is \textbf{0.544}, and the best AFT model is
LogNormalAFT (AIC 3{,}542.5). Operational 12- and 24-month indicators are
available for 1{,}204 contracts, with median probabilities 0.0198 and 0.0624.
The headline caveat: \texttt{event} is a proxy recurrence outcome, not BOAMP
ground truth; real BOAMP precision and recall are not directly observable.
\end{abstract}
...
row-identical to the later official selection
\texttt{data/processed/boamp\_phase2\_survival\_method\_m0\_balanced.csv}. Its
...
valid fits; \textbf{the official L3 Cox result reported throughout this
document is the reduced-spec C-index of 0.544} (AFT AIC 3{,}542.5), because it
is the specification used for the final method-comparison decision. The
richer 0.592 model is retained only as a secondary, category-aware fit. The
Schoenfeld residual test on the official 0.544 spec flags a PH violation for
\texttt{declared\_duration\_months} ($p<10^{-24}$) and passes for
\texttt{dur\_was\_imputed} and \texttt{start\_year} --- the same pattern as
the pre-calibration model, for which the log-normal AFT above is used as the
PH-assumption-free cross-check.
...
compares the current calibrated rule (M0) with two literature-supported
...
manual TP/FP labels added for M2 where available. This is a comparison under
no legal ground truth: real BOAMP precision and recall are not directly
observable.

\begin{table}[H]
\centering
\caption{Latest real BOAMP method-comparison result.}
...
Across all synthetic scenarios, M0 balanced scores precision 0.575 and recall
0.568; M1/M2 balanced score precision 0.679 and recall 0.642. However, the
final recommendation remains \textbf{M0 balanced as the current main method},
with M2 balanced retained as the best alternative for targeted manual review.
The active-learning output is a 150-pair review queue: 25 pairs in each of six
review categories (near-threshold, high-score generic CPV, event=0 with high
nearest candidate, low top1--top2 margin, high text with CPV mismatch, and
same-buyer/same-CPV weak-text).

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/validation/method_comparison_synthetic_precision_recall}
\caption{Synthetic precision and recall for M0, M1, and M2. The synthetic
benchmark supports comparing methods, but it does not prove real BOAMP truth.}
...
M2 balanced, and M0 strict. M0 balanced remains the selected method.}
...
Renewal linking (event reconstruction)
  & \textcolor{done}{Done}
  & M0 balanced: 269/1,210 proxy events (22.2\%)
  & \nolinkurl{boamp_renewal_linking_quality/} \\[3pt]
Survival dataset construction
  & \textcolor{done}{Done}
  & 1,210 rows $\times$ 33 cols
  & \nolinkurl{data/processed/boamp_phase2_survival_method_m0_balanced.csv} \\[3pt]
...
Parametric AFT models
  & \textcolor{done}{Done}
  & LogNormalAFT best (AIC 3,542.5)
  & same notebook \\[3pt]
Sensitivity analysis
  & \textcolor{done}{Done}
  & M2 balanced alternative; M0 strict high-confidence sensitivity
  & \nolinkurl{reports/tables/validation/method_survival_comparison.csv} \\[3pt]
Robustness checks
  & \textcolor{done}{Done}
  & Synthetic benchmark, negative controls, threshold sweep, manual audit
  & \nolinkurl{reports/tables/validation/} \\[3pt]
Operational risk indicators
  & \textcolor{done}{Done}
  & 1,204 contracts scored; median p12=0.0198, p24=0.0624
  & \nolinkurl{reports/tables/survival/renewal_risk_12_24_months_calibrated_balanced.csv} \\[3pt]
High-risk contract/buyer/segment tables
  & \textcolor{done}{Done}
  & Top-20 contracts by 12-month recurrence probability, plus buyer- and
    segment-level rankings, under M0 balanced
  & \nolinkurl{reports/tables/survival/top20_renewal_risk_calibrated_balanced.csv},
    \nolinkurl{buyer_renewal_risk_ranking_calibrated_balanced.csv},
    \nolinkurl{segment_renewal_risk_ranking_calibrated_balanced.csv} \\[3pt]
...
present on the official M0 balanced dataset: Kaplan--Meier curves, a validated
...
M0 balanced} method. The old 665-event result appears only as a historical
pre-calibration baseline and should not be read as the current main result.

\subsection{Renewal linking}

\begin{table}[H]
\centering
\caption{Current proxy-recurrence outcome under selected M0 balanced method.}
\label{tab:link}
\small
\begin{tabular}{@{}lr@{}}
\toprule
...
Proxy recurrence events (\texttt{event}~$=1$) & 269 \quad(22.2\%) \\
Censored/no observed link under rule (\texttt{event}~$=0$) & 941 \quad(77.8\%) \\
Old pre-calibration baseline & 665 \quad(55.0\%; historical only) \\
Best alternative candidate & M2 balanced: 256 events \quad(21.2\%) \\
\bottomrule
\end{tabular}
\end{table}

The broad sensitivity gives 490 events (40.5\%) and the strict sensitivity gives
79 events (6.5\%). The probabilistic and active-learning alternatives are not
selected as the main method, but they are documented for review: M1 balanced has
278 real proxy events and M2 balanced has 256.
...
Under M0 balanced, the survival dataset has 1{,}210 rows and 269 proxy events.
The Kaplan--Meier median is not reached, meaning fewer than 50\% of contracts
experience the calibrated proxy event before censoring. Fixed-horizon survival
is 95.9\% at 12 months, 92.3\% at 24 months, 83.2\% at 48 months, and 75.2\%
at 60 months. The Cox C-index is 0.544 (see the two-specification note above).
LogNormalAFT is the best AFT model by AIC (3{,}542.5), ahead of WeibullAFT
(3{,}570.2) and LogLogisticAFT (3{,}580.0) --- WeibullAFT was tested but not
retained since it does not have the lowest AIC. Operational 12/24-month
indicators are available under M0 balanced for 1{,}204 contracts, with median
probabilities 0.0198 and 0.0624 and expected counts 27.5 and 82.3.
...
by AIC (3{,}542.5). These outputs translate BOAMP proxy recurrence signals ---
