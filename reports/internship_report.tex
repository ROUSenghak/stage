...
7{,}176.6). The fitted model is translated into 12- and 24-month renewal-risk
...
Right-censored (\texttt{event}~$=0$) & 395 \quad(35.9\%) \\
...
(Table~\ref{tab:sens}). \textbf{Main unlinked failure mode:} 331 of 395 unlinked
contracts (83.8\%, rounded 84\%) have no temporal partner in the search window --- a true absence
...
imputation status ($p<0.001$), not by technology category ($p=0.182$).
...
raises it (HR 1.094 per year). No category or procedure effect. The positive
...
\paragraph{Parametric AFT models.} Log-normal fits best (AIC 7{,}176.6,
