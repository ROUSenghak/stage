...
%% Covers: BOAMP dataset (section 1), DECP (section 2), processed datasets (section 3),
...
% ============================================================
\section{BOAMP -- Bulletin Officiel des Annonces des Marchés Publics}
% ============================================================

\subsection{Présentation générale}

Le BOAMP est le bulletin officiel des annonces de marchés publics français.
Contrairement au DECP, qui enregistre le \emph{contrat signé}, le BOAMP
enregistre des \emph{avis} publiés aux différentes étapes de la procédure
d'attribution : appel d'offres, attribution, rectificatif, modification ou
pré-information. Cette granularité événementielle est indispensable pour
reconstituer les paires (annonce initiale, résultat d'attribution) qui
servent de signal de renouvellement dans l'analyse de survie.

Les avis sont publiés sur \texttt{boamp.fr} et interrogeables via l'API de
recherche décrite dans le document partenaire (section~1 de l'équipe).

\medskip
\textbf{Fichier utilisé}

La récupération s'effectue par des appels paginés à l'API de recherche du
BOAMP, filtrés par département de l'acheteur et par code CPV ou descripteur.
L'API retourne des documents JSON dont chacun comporte un champ
\texttt{donnees} imbriqué contenant le corps complet de l'avis.
Deux formats de \texttt{donnees} coexistent : le format \texttt{legacy}
(2015--2023, structure à clés françaises) et le format \texttt{eforms}
(2024, format UBL européen harmonisé). Les fichiers bruts sont stockés dans
\texttt{data/raw/boamp\_full/}.

\medskip
\textbf{Volume et couverture}

\begin{itemize}
  \item[---] Base totale de l'API : plusieurs millions d'avis pour l'ensemble
    de la France et de tous les secteurs depuis environ 2000
  \item[---] Couverture temporelle dense : à partir de \textbf{2015}
    (format de l'API stable, couverture fiable)
  \item[---] Corpus de travail : \textbf{3\,181 avis} relatifs aux marchés
    numériques en Pays de la Loire (2015--2024), téléchargement complet
    sans stratification (177--398 avis par année)
  \item[---] Répartition des formats dans le corpus : 2\,893 avis
    \texttt{legacy} (91\,\%) et 288 avis \texttt{eforms} (9\,\%)
\end{itemize}

\medskip
\textbf{Filtres appliqués pour constituer le corpus}

\begin{itemize}
  \item[---] Département de l'acheteur : 44, 49, 53, 72, 85 (Pays de la Loire)
  \item[---] Code CPV ou descripteur : premiers chiffres dans \{48, 72, 32, 35\}
    (marchés numériques / informatiques)
  \item[---] Année de parution : 2015 à 2024
  \item[---] Téléchargement complet (sans stratification ni limite par année)
\end{itemize}
...
\medskip
\textbf{Identification de l'avis}

\begin{itemize}
  \item[---] \texttt{idweb} : identifiant unique de l'avis attribué par le
    BOAMP (format \texttt{AA-NNNNN})
  \item[---] \texttt{nature} : type d'avis (\texttt{APPEL\_OFFRE},
    \texttt{ATTRIBUTION}, \texttt{RECTIFICATIF}, \texttt{MODIFICATION},
    \texttt{PRE-INFORMATION})
  \item[---] \texttt{nature\_libelle} : libellé lisible du type d'avis
    (ex.\ \textit{Avis de marché}, \textit{Résultat de marché})
  \item[---] \texttt{famille} : famille de publication -- \texttt{BOAMP}
    pour les marchés nationaux, \texttt{JOUE} pour les marchés soumis à
    publication au Journal officiel de l'Union européenne
  \item[---] \texttt{etat} : statut de l'avis (\texttt{INITIAL} = original,
    \texttt{RECTIFICATIF} = version corrigée)
  \item[---] \texttt{donnees\_format} : format du champ imbriqué
    \texttt{donnees} (\texttt{legacy} ou \texttt{eforms})
\end{itemize}
...
\begin{itemize}
  \item[---] \texttt{dateparution} : date de publication de l'avis ;
    couverture 100\,\% -- point d'ancrage temporel du corpus BOAMP (date de
    parution de l'avis, non de la signature du contrat)
  \item[---] \texttt{datefindiffusion} : date de fin de diffusion en ligne
  \item[---] \texttt{datelimitereponse} : date limite de remise des offres
    par les candidats (uniquement pour les appels d'offres)
\end{itemize}
...
\begin{itemize}
  \item[---] \texttt{nomacheteur} : nom de l'acheteur en texte libre ;
    couverture 100\,\% -- les variantes orthographiques (accents,
    abréviations, suffixes institutionnels) nécessitent une normalisation
  \item[---] \texttt{code\_departement} : liste des codes de département
    associés à l'avis (lieu de l'acheteur et/ou lieu d'exécution)
  \item[---] \texttt{buyer\_siret} : SIRET de l'acheteur extrait de
    \texttt{donnees} ; disponible uniquement dans les avis \texttt{eforms}
    2024 (couverture globale 8\,\% du corpus)
\end{itemize}
...
\begin{itemize}
  \item[---] \texttt{objet} : intitulé du marché en texte libre ; couverture
    100\,\% -- plus riche et plus long que dans le DECP, source principale
    pour les traitements NLP de la phase 2
  \item[---] \texttt{cpv\_principal} : code CPV principal ; couverture
    97,8\,\% dans le corpus
  \item[---] \texttt{cpv\_all} : liste complète des codes CPV mentionnés dans
    l'avis (marchés multi-postes)
  \item[---] \texttt{type\_procedure} : type de procédure structuré
    (\texttt{OUVERT}, \texttt{PROCEDURE\_ADAPTE}, \texttt{NEGOCIE}\ldots)
  \item[---] \texttt{type\_marche} : catégorie du marché (\texttt{SERVICES},
    \texttt{FOURNITURES}, \texttt{TRAVAUX})
  \item[---] \texttt{descripteur\_libelle} : mots-clés thématiques assignés
    par le BOAMP (ex.\ \textit{Informatique -- prestations de services})
\end{itemize}

\medskip
\textbf{Attribution et renouvellement}

\begin{itemize}
  \item[---] \texttt{titulaire} : nom du ou des attributaires ; présent
    dans les avis d'attribution (couverture globale 37\,\% du corpus)
  \item[---] \texttt{annonce\_lie} : liste des identifiants \texttt{idweb}
    des avis liés -- par exemple l'appel d'offres correspondant à une
    attribution, ou l'avis initial d'un rectificatif ; couverture 46\,\% --
    clé principale pour le chaînage des procédures et la détection du
    renouvellement
  \item[---] \texttt{date\_attribution} : date d'attribution du marché,
    extraite du champ \texttt{donnees} dans les avis d'attribution
    (couverture 42\,\% de l'ensemble du corpus)
\end{itemize}

\medskip
\textbf{Champ imbriqué \texttt{donnees}}

Le champ \texttt{donnees} contient le corps complet de l'avis encodé en JSON.
Son contenu varie selon le format de publication :

\begin{itemize}
  \item[---] \textbf{Format \texttt{legacy}} (460 avis, 2015--2023) : structure
    à clés françaises. Sections typiques : \texttt{IDENTITE} (données de
    l'acheteur -- dénomination, code postal, ville), \texttt{OBJET} (code CPV,
    titre, durée), \texttt{PROCEDURE} (type de procédure, critères
    d'attribution), \texttt{ATTRIBUTION} (titulaire, montant, date). C'est
    dans ce champ que sont extraits le montant, la durée et la date
    d'attribution renseignés dans le CSV nettoyé.
  \item[---] \textbf{Format \texttt{eforms}} (40 avis, 2024) : structure UBL
    européenne, clé racine \texttt{EFORMS}, hiérarchie plus complexe issue de
    la directive européenne. Contient le SIRET de l'acheteur, absent des
    avis legacy.
\end{itemize}

\subsection{Comparaison de couverture avec le DECP}
\label{sec:boamp-comparaison}

Le tableau de comparaison champ par champ entre BOAMP et DECP est présenté à
la section~\ref{sec:decp-comparaison}. Les apports distinctifs du BOAMP dans
le contexte du projet sont résumés ci-dessous.
...
\midrule
SIRET acheteur          &  9\,\%   & 100\,\% \\
Montant (EUR)           & 43\,\%   &  93\,\% \\
Durée (mois)            & 48\,\%   &  99\,\% \\
Date notification       &  ---     & 100\,\% \\
Texte du marché (objet) & 100\,\% & 100\,\% \\
Type d'avis             & 100\,\% &  ---    \\
Liens de renouvellement & 35\,\%  &  ---    \\
\bottomrule
\end{tabular}
\end{center}

\begin{itemize}
  \item[---] \textbf{Couverture temporelle pleine 2015--2024} : le corpus
    BOAMP couvre la décennie entière, tandis que le DECP n'est dense qu'à
    partir de 2019
  \item[---] \textbf{Granularité événementielle} : la distinction appel
    d'offres / attribution est structurelle dans le BOAMP, absente du DECP
  \item[---] \textbf{Texte libre riche} : l'objet BOAMP est plus long et
    plus détaillé, adapté aux approches NLP
  \item[---] \textbf{Liens de chaînage} : le champ \texttt{annonce\_lie}
    fournit des paires appel--attribution, signal direct de renouvellement
    absent du DECP
\end{itemize}

\subsection{Exemple d'enregistrement BOAMP}
...
\begin{lstlisting}[language={}]
{
  ...
  "famille"           : "JOUE",
  "etat"              : "INITIAL",
  "dateparution"      : "2016-02-02",
  "datefindiffusion"  : "2016-03-11",
  "datelimitereponse" : "2016-03-11T11:00:00+00:00",
  "nomacheteur"       : "INRA Angers Nantes",
  "code_departement"  : ["44"],
  "objet"             : "prestation d'hebergement, support technique et
    assistance pour la plateforme de teleformation moodle de l'INRA",
  "type_procedure"    : "OUVERT",
  "type_marche"       : ["SERVICES"],
  "titulaire"         : null,
  "annonce_lie"       : null,
  "donnees_format"    : "legacy",
  "donnees" : {
    "IDENTITE"  : { "DENOMINATION" : "INRA Angers Nantes",
                    "CP"           : "44316",
                    "VILLE"        : "Nantes" },
    "OBJET"     : { "CPV"          : { "PRINCIPAL" : "72415000" },
                    "DUREE_DELAI"  : { "DUREE_MOIS" : "36" } },
    "PROCEDURE" : { "TYPE_PROCEDURE" : { "OUVERT" : "" } },
    ...
  },
  "url_avis" : "https://www.boamp.fr/pages/avis/?q=idweb:16-14558"
}
\end{lstlisting}
...
\begin{itemize}
  \item[---] Corpus principal d'EDA (semaine 1) : les 3\,181 avis constituent
    le socle de l'exploration descriptive de la structure des marchés numériques
    en Pays de la Loire sur la période 2015--2024
  \item[---] Source NLP (phase 2) : le champ \texttt{objet}, plus riche que
    son équivalent DECP, sera la source de texte pour la vectorisation et la
    classification des marchés
  \item[---] Détection de renouvellement : le couple \texttt{APPEL\_OFFRE} /
    \texttt{ATTRIBUTION} lié via \texttt{annonce\_lie} constitue un signal de
    renouvellement direct, complémentaire de l'algorithme DECP
    (section~\ref{sec:renewal})
  \item[---] Enrichissement croisé : la table \texttt{buyer\_bridge.csv} relie
    les acheteurs BOAMP (clé nom normalisé) aux acheteurs DECP (clé SIRET)
    pour les analyses longitudinales multi-sources
\end{itemize}
...
\breakcode{data.gouv.fr} (identifiant de ressource
\breakcode{11cea8e8-df3e-4ed1-932b-781e2635e432}).
...
\begin{lstlisting}[language={}]
{
  ...
  "objet"                  : "Infog\'{e}rance du syst\`{e}me d'information",
  "codeCPV"                : "72000000",
  "procedure"              : "Proc\'{e}dure adapt\'{e}e",
  "montant"                : 140000.0,
  "dureeMois"              : 48.0,
...
% ------------------------------------------------------------
\subsection{\texttt{boamp\_full\_clean.csv} -- Corpus BOAMP complet nettoyé}
% ------------------------------------------------------------

\textbf{Contenu}

Corpus complet de \textbf{3\,181 avis} BOAMP couvrant la période 2015--2024 en
Pays de la Loire, domaine numérique (CPV 48 / 72 / 32 / 35), téléchargement
exhaustif sans stratification, mixant les types d'avis (appels d'offres,
attributions, etc.). Le fichier est le résultat de la mise à plat
(\textit{flatten}) du champ \texttt{donnees} et des transformations de
nettoyage.
...
\begin{itemize}
  \item[---] Dimensions : 3\,181 lignes × 44 colonnes
  \item[---] Format : CSV (séparateur virgule, encodage UTF-8)
  \item[---] Couverture temporelle : 2015--2024 (177--398 avis par année)
\end{itemize}
...
\begin{itemize}
  \item[---] \texttt{APPEL\_OFFRE} : 1\,933 avis (61\,\%)
  \item[---] \texttt{ATTRIBUTION} : 1\,081 avis (34\,\%)
  \item[---] \texttt{RECTIFICATIF} : 150 avis (5\,\%)
  \item[---] \texttt{MODIFICATION} / \texttt{PRE-INFORMATION} / autres : 17 avis
    ($<$1\,\%)
\end{itemize}
...
\begin{itemize}
  \item[---] \texttt{donnees\_format = legacy} : 2\,893 avis (91\,\%, format BOAMP
    classique, clés françaises : \texttt{IDENTITE}, \texttt{OBJET},
    \texttt{PROCEDURE}\ldots)
  \item[---] \texttt{donnees\_format = eforms} : 288 avis (9\,\%, format UBL
    européen 2024, clé racine \texttt{EFORMS}, structure XML imbriquée)
\end{itemize}
...
\begin{itemize}
  ...
  \item[---] \texttt{amount\_raw}, \texttt{flag\_amount\_zero},
    ...
    et montant nettoyé (taux de renseignement après nettoyage : 41\,\%)
  \item[---] \texttt{duration\_raw\_boamp}, \texttt{flag\_duration\_suspect},
    ...
    (taux de renseignement après nettoyage : 47\,\%)
...
\textbf{Indicateur} & \textbf{Montant (EUR)} &
  \textbf{Durée (mois)} \\
\midrule
Observations valides & 1\,306 & 1\,506 \\
Moyenne              & 841\,065 & 30,9 \\
Médiane              & 400\,000 & 24,0 \\
Min                  & 2\,000   & 1,0 \\
Max                  & 9\,750\,000 & 120,0 \\
\bottomrule
\end{tabular}
...
\begin{tabular}{lcc}
\toprule
\textbf{Catégorie} & \textbf{BOAMP (3\,181)} &
  \textbf{DECP (3\,039)} \\
\midrule
IT Services \& Consulting         & 994 (31\,\%) & 1\,321 (43\,\%) \\
Software \& Applications          & 599 (19\,\%) &   608 (20\,\%) \\
Telecom \& Networks               & 552 (17\,\%) &   581 (19\,\%) \\
Cybersecurity                     & 354 (11\,\%) &   337 (11\,\%) \\
Digital Workplace \& Collaboration & 158 (5\,\%) &   110 (4\,\%) \\
Unknown                           & 266 (8\,\%) &     0 (0\,\%) \\
Autres (CAT05--CAT10)             & 258 (8\,\%) &    82 (3\,\%) \\
\bottomrule
\end{tabular}
\end{center}
