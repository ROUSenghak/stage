...
DATA_PATH = ROOT / "data" / "processed" / "boamp_phase2_survival_method_m0_balanced.csv"
TABLE_OUT = ROOT / "reports" / "tables" / "survival" / "m0_balanced_cox_ph_assumption_test.csv"
FIGURE_OUT = ROOT / "reports" / "figures" / "survival" / "m0_balanced_coxph_loglog_dur_imputed.png"
...
print(f"Official M0 balanced Cox C-index: {cph.concordance_index_:.6f}")
...
ph_summary["rule_name"] = "M0_balanced"
...
ax.set_title("Log-log PH check — dur_was_imputed\n(M0 balanced, official Cox spec)")
ax.legend()
...
print(f"\nCovariates violating PH (p<0.05): {violated if violated else 'none'}")
